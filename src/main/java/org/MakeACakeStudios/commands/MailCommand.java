...
public class MailCommand implements Command {
    ...
    private void handle(@NotNull Player sender, @NotNull OfflinePlayer target, String message) {
        ...
        String prefix = PlayerDataStorage.instance.getPlayerPrefixByName(sender.getName());
        String suffix = PlayerDataStorage.instance.getPlayerSuffixByName(sender.getName());

        MailStorage.instance.addMessage(target.getName(), prefix, sender.getName(), suffix, message);
        ...
        if (targetPlayer != null) {
            ...
            targetPlayer.sendMessage(MiniMessage.miniMessage().deserialize("<green><click:run_command:/mailread " + lastId + "><hover:show_text:'Нажмите <green>ЛКМ</green>, чтобы прочитать последнее сообщение от " + prefix + sender.getName() + suffix + ".'>У вас есть новое сообщение!</click></green>"));
